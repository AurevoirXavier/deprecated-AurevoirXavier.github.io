...
首先，`[T]` 不是 `Vec`，而是[T]。vec!(1,2,3,4,5)的类型为vec 。不能有类型为[T]的值;它总是在某种间接的背后。
让我们后退一步。
Rust希望能够有效地获取并传递数组的切片。这样做的一个好方法是传递(&T, usize)的bundle:一个指向第一个元素的指针，以及元素的数量。您可以从任何连续存储中创建其中一个，无论它来自何处。例如，&[T]是其他存储的一个借来的片。这就提出了“如果&X是指向X的指针，那么[T]本身意味着什么?”
[T]然后是“一些连续存储的Ts”的类型。它没有说明它们存储在哪里，谁拥有它们，如何管理它们，等等，只是某个数字(包括0)存在于某个地方。您需要[T]，因为您可以构建它来创建其他更有用的类型，这些类型对于如何存储这些T是有意义的。&[T]说它们是被别人借来并拥有的(可能是静态数据段中的文字)。Box<[T]>说，你拥有它们，并且拥有对它们的独占访问权。Rc<[T]>说所有权是共享的。[T;5)恰好是5个拥有的Ts。
Vec存在的原因是，一个很常见的事情是建立数组元素的元素。它基本上是Box<[T]>，但是它保留了一些额外的空间，以使它更高效。你可以对它进行切片以获取&[T]因为你可以对所有只是连续存储的东西进行切片以获取&[T]。
字符串和str是完全相同的，除了“必须是有效的UTF-8”之外。
所以[T]/str是基本的构建块。Vec和String都是建立在它们之上的东西，因为[T]和str本身不会做很多事情。